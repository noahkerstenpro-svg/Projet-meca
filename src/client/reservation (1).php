...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Vérification week-end côté serveur
    $jourSemaine = date('N', strtotime($date)); // 6 = samedi, 7 = dimanche
    if ($jourSemaine >= 6) {
        $erreur = 'Les réservations ne sont pas disponibles le week-end.';
    } elseif ($vehicule && $date && $heure && $probleme) {
...
async function majCreneaux() {
    ...
    if (date && estWeekend(date)) {
        dateInput.setCustomValidity('Les réservations ne sont pas disponibles le week-end.');
        dateInput.reportValidity();
        dateInput.value = '';
        document.getElementById('heure').innerHTML = '<option value="" disabled selected>Choisir une heure</option>';
        return;
    }
    ...
    // Récupérer les créneaux pris pour cette date
    const resp = await fetch('creneaux_pris.php?date=' + date);
    const pris = await resp.json(); // ex: ["09:00","14:00"]
