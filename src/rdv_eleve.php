...
function couleurHeure($heure) {
    return (int)substr($heure, 0, 2) < 12 ? 'matin' : 'aprem';
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Agenda RDV — Espace Élève</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .stat-value { font-size: 32px; font-weight: bold; color: #eb5e00; }
        ...
        .jour-date { font-size: 18px; font-weight: bold; color: #525151; }
        .jour-date .jour-nom { color: #1e3a8a; }
        ...
        .jour-line { flex: 1; height: 1px; background: #ddd; }

        /* ── CARTE RDV — lecture seule (3 colonnes, pas de boutons) ── */
        ...
        .rdv-client { font-size: 15px; font-weight: bold; color: #111; }
        .rdv-vehicule { font-size: 13px; color: #888; }
        ...
        .rdv-prestation.autre { background: #f0f0f0; color: #555; }
        ...
        .rdv-prix { font-size: 18px; font-weight: bold; color: #eb5e00; }
        .rdv-prix.gratuit { font-size: 12px; color: #aaa; font-weight: normal; }
        ...
        .rdv-card.or-valide { opacity: 0.6; }
        ...
        /* ── VIDE / ERREUR ── */
        .vide { text-align: center; padding: 60px 20px; color: #bbb; font-size: 16px; }
...
<div class="search-bar">
    <input class="search-input" type="text" id="recherche"
           placeholder="Rechercher un client, véhicule, prestation…" oninput="filtrer()">
        ...
        <?php foreach ($rdvs as $r): ?>
            ...
            <div class="rdv-card <?= $moment ?> <?= ($r['statut'] ?? '') === 'valide' ? 'or-valide' : '' ?>"
                ...
                <div class="rdv-body">
                    ...
                    <div class="rdv-vehicule"><?= htmlspecialchars($r['vehicule']) ?></div>

                    <?php if ($r['prestation_nom']): ?>
                        <div class="rdv-prestation">🔧 <?= htmlspecialchars($r['prestation_nom']) ?></div>
                    <?php elseif ($r['Probleme']): ?>
                        <div class="rdv-prestation autre">✏️ <?= htmlspecialchars($r['Probleme']) ?></div>
                    <?php endif; ?>
                </div>
...
function filtrer() {
    ...
    document.querySelectorAll('.jour-bloc').forEach(jour => {
        ...
        jour.querySelectorAll('.rdv-card').forEach(card => {
            ...
            card.style.display = (matchMoment && matchStatut && matchSearch) ? '' : 'none';
            if (matchMoment && matchStatut && matchSearch) visibles++;
        });
